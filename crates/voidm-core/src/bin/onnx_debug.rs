...
async fn main() -> anyhow::Result<()> {
    ...
    let prompt = std::env::args()
        .nth(2)
        .unwrap_or_else(|| "Expand query: python ->".to_string());
    ...
    let api = hf_hub::api::tokio::ApiBuilder::new()
        .with_cache_dir(cache_dir.parent().unwrap().to_path_buf())
        .build()
        .map_err(|e| anyhow::anyhow!("Failed to build API: {}", e))?;
    ...
    // Tokenize prompt
    println!("\n=== Tokenization ===");
    let encoding = tokenizer.encode(prompt.as_str(), true).unwrap();
    let token_ids: Vec<i64> = encoding.get_ids().iter().map(|&id| id as i64).collect();
    println!(
        "Tokens: {:?}",
        &token_ids[..std::cmp::min(10, token_ids.len())]
    );
    ...
    for step in 0..max_tokens {
        ...
        let attention_mask_tensor =
            Tensor::<i64>::from_array(([1, seq_len], attention_mask.into_boxed_slice())).unwrap();
        ...
        let logits_tensor = logits_value.try_extract_tensor::<f32>()?;
        let (_shape, logits_data) = logits_tensor;

        let vocab_size = logits_data.len() / seq_len;
        println!(
            "Step {}: vocab_size={}, logits_len={}",
            step,
            vocab_size,
            logits_data.len()
        );
        ...
        println!(
            "  Generated token: {} (logit: {:.4})",
            next_token, last_logits[next_token as usize]
        );

        if next_token == 50256 || next_token == 2 {
            // EOS tokens
            println!("  [EOS]");
            break;
        }
    ...
    // Decode
    println!("\n=== Decoding ===");
    let decoded = tokenizer.decode(&generated, true).unwrap();
    println!("Generated: {}", decoded);
    println!("\nFull output: {} {}", prompt, decoded);
